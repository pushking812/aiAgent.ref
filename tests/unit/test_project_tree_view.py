...
class TestProjectTreeViewAdditional2:
    ...
    def test_clean_search_path(self, project_tree_view):
        ...
        # 5. ВОЗМОЖНО: заменяет множественные точки на одну
        ...
        test_cases = [
            ...
            ("app..main", "app.main"),  # Двойные точки В СЕРЕДИНЕ - заменяются на одну!
            ("  app  .  main  ", "app.main"),
            ("🔹app📦main📝", "appmain"),
            ("APP.MAIN", "app.main"),
            (".app.main.", "app.main"),  # Точки по краям удаляются
            ("..app..main..", "app.main"),  # Точки по краям удаляются, двойные в середине заменяются на одну
        ]
